...
public class ListenersTest {
	...
	public static void main(String[] args) throws InterruptedException, FileNotFoundException, IOException {
		...
		if (browserName.equals("chrome")) {
			...
			driverOriginal = new ChromeDriver();// launch browser

		} else if (browserName.contentEquals("firefox")) {
			System.setProperty("webdriver.gecko.driver", projectPath + "\\resources\\drivers\\geckodriver.exe");

			driverOriginal = new FirefoxDriver();// launch browser

		} else {

			System.setProperty("webdriver.edge.driver", projectPath + "\\resources\\drivers\\msedgedriver.exe");

			driverOriginal = new EdgeDriver();// launch browser

		}

		Listners listener = new Listners();

		WebDriver driver = new EventFiringDecorator(listener).decorate(driverOriginal);
